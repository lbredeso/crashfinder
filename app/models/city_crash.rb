class CityCrash
  ...
  def self.map
    <<-MAP
      function() {
        if (this.location && this.city_township_name) {
          emit({
            city: this.city_township_name, year: this.year
          }, {
  ...
  def self.build
    Crash.collection.map_reduce(map, reduce, { :out => "city_crashes", :finalize => finalize })
  end
